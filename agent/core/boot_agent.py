...
class BootAgent():
    ...
    def resolve_factory_injections(self):
        ...
        agent_name = self.command_line_args.get("agent_name", "unknown")
        base_factory_path = f"agent.{agent_name}.factory"

        for key, value in factories.items():
            if not isinstance(key, str) or "." not in key:
                self.log(f"[FACTORY-SKIP] Invalid or unqualified factory key: {key}")
                continue

            try:
                module_path = f"{base_factory_path}.{key.replace('.', '.')}"
                self.log(f"[FACTORY] Trying: {module_path}")
                mod = importlib.import_module(module_path)
                mod.attach(self, value)
                self.log(f"[FACTORY] Loaded: {module_path}")
            except Exception as e:
                self.log(f"[FACTORY-ERROR] {module_path} → {e}")
