...
"""Authenticated manual reader for encrypted MatrixSwarm Discord alerts."""
...
ENCRYPTED_ALERT_HANDLER = "discord_relay.alert"
DISCORD_ALERT_HASH_SUFFIX = "discord-relay-message"


def decode_alert_envelope(message_text: str) -> dict:
    """Recover a Base64 JSON alert envelope from copied Discord text."""
    if not isinstance(message_text, str) or not message_text.strip():
        raise ValueError("Paste an encrypted Discord alert first.")
    ...
    if marker_index >= 0:
        ...
        if payload_lines:
            # Discord clients can wrap a copied Base64 block. Try the entire
    ...
    raise ValueError("The Discord alert is not a valid Base64 JSON envelope.") from last_error
...
def _optional_timestamp(value, field_name: str):
    ...
    try:
        ...
        raise ValueError(
            f"The signed Discord alert {field_name} timestamp is invalid."
        ) from error
...
def decrypt_alert_message(
    ...
    """Authenticate and decrypt an archived Discord alert.
    ...
    if not isinstance(trusted_serial, str) or not trusted_serial.strip():
        raise ValueError("The current Discord agent has no trusted serial.")
    ...
    if not isinstance(serial, str) or not hmac.compare_digest(
        ...
        raise ValueError("The Discord alert serial does not match the current agent.")

    signature = signed_wrapper.get("sig")
    if not isinstance(signature, str) or not signature.strip():
        raise ValueError("The signed Discord alert signature is missing.")

    expected_hash = hashlib.sha256(
        f"{serial}{DISCORD_ALERT_HASH_SUFFIX}".encode("utf-8")
    ).hexdigest()
    ...
    if not isinstance(message_hash, str) or not hmac.compare_digest(
        ...
        raise ValueError("The signed Discord alert purpose hash is invalid.")

    if not sender_pubkey or not phoenix_privkey:
        raise ValueError("The current Discord agent certificate pair is incomplete.")
    ...
    if not isinstance(encrypted_content, dict):
        raise ValueError("The encrypted Discord alert content is missing.")
    ...
    if isinstance(clear_payload, bytes):
        try:
            ...
            raise ValueError("The decrypted Discord alert is not UTF-8 text.") from error
    if isinstance(clear_payload, str):
        try:
            clear_payload = json.loads(clear_payload)
        except json.JSONDecodeError as error:
            raise ValueError("The decrypted Discord alert is not valid JSON.") from error

    if not isinstance(clear_payload, dict):
        raise ValueError("The decrypted Discord alert payload is invalid.")
    if clear_payload.get("handler") != ENCRYPTED_ALERT_HANDLER:
        raise ValueError("The decrypted payload is not a Discord relay alert.")

    content = clear_payload.get("content")
    if not isinstance(content, dict):
        raise ValueError("The decrypted Discord alert message is missing.")
...
class DiscordRelay(PhoenixPanelInterface):
    """Paste, authenticate, and decrypt encrypted Discord relay alerts."""
    ...
    def _build_layout(self):
        ...
        instructions = QLabel(
            "Paste the complete encrypted Discord alert below, then click "
        ...
        message_box = QGroupBox("📡 Decrypted Discord Alert")
    ...
    def _get_current_alert_keys(self):
        ...
        if not uid:
            raise RuntimeError("Current DiscordRelay agent has no universal_id.")
    ...
    def _decrypt_alert(self):
        ...
        try:
            ...
            emit_gui_exception_log("DiscordRelay._decrypt_alert", error)
            QMessageBox.warning(
                self,
                "Encrypted Discord Alert Rejected",
